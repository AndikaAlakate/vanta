<section id="daftar-baru hidden">
    <div class="container mt-5">
        <div class="row">
            <div class="col text-center pb-4 pt-4">
                <h2>Daftar Buku Terbaru</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-7.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4 class="card-text">Judul Buku 1</h4>
                        <!-- <hr> -->
                        <p class="card-text">Penulis: Penulis 1</p>
                        <p class="card-text">Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-8.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-9.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-10.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-11.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-12.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-13.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-14.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 mb-3">
                <div class="card shadow">
                    <div class="card-img">
                        <img src="e-book/e-book-15.webp" alt="Buku 1" class="img-fluid card-img-top">
                    </div>
                    <div class="card-body">
                        <h4>Judul Buku 1</h4>
                        <p>Penulis: Penulis 1</p>
                        <p>Tahun Terbit: 2023</p>
                        <a href="detail-buku-1.html" class="btn btn-primary btn-sm">Detail Buku</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>